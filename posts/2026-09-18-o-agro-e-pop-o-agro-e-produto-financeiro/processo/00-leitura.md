...
<!-- Cópia de trabalho de _arquivo/drafts/2026-09-18_O_Agro_é_POP_O_Agro_é_Produto_Financeiro.md — o original permanece intocado. Título e linha_editorial confirmados com o autor na etapa 0 (front-matter do rascunho original não trazia esses campos em YAML válido). -->
...
Todos os marcadores do rascunho original (colchetes e comentários HTML), classificados pelos
quatro padrões da etapa 0. Nenhum foi resolvido nesta etapa — "resolução" fica vazio até a
etapa responsável tratar.

1. **[Estrutural]** `[LINHA EDITORIAL]` — ausente do front-matter original (não estava em YAML válido). Resolvido nesta sessão, por pergunta direta ao autor: **Notas de um Professor**.
   Resolução: confirmado com o autor em 2026-09-18, antes da etapa 1.

2. **[Nota de conteúdo]** Tese central do bloco "NOTAS DO AUTOR PARA O PIPELINE": *"a LCA é uma dívida do banco, não do agro. O lastro rural define para onde o dinheiro vai; quem paga o investidor é o balanço do banco. [...] a isenção não é generosidade: é um benefício fiscal repartido entre banco e investidor."*
   Resolução: (etapa 1 — vira a tese em uma frase do briefing)

3. **[Instrução de escrita]** Fio condutor: *"separar três perguntas que se confundem — (1) quanto a LCA rende para quem compra; (2) quanto custa para quem emite; (3) quanto ela vale hoje."*
   Resolução: já implementado na estrutura do próprio rascunho (títulos de seção seguem essa ordem) — confirmar na etapa 2 que a estrutura final preserva isso.

4. **[Nota de conteúdo]** Insumo para a Ficha de Saída (etapa 2) — cinco competências de saída listadas pelo autor: explicar por que o risco é do banco; calcular break-even LCA×CDB; precificar LCA prefixada bullet e descrever marcação de pós-fixada; distinguir taxa contratual de taxa de mercado; diferenciar LCA de CRA.
   Resolução: (etapa 2 — usar como campo 1, "competência de saída", da Ficha de Saída)

5. **[Nota de conteúdo]** Material aproveitável do "draft v2" (conversa anterior sobre LCA): tabela de break-even, FGC sobre principal + juros, FGC × FGCoop, direcionamento de 60%, MP 1.303 e IRPFM — "trouxe aqui só o que serve ao fio das três perguntas".
   Resolução: (etapa 1/4 — confirmar que nada desse material ficou de fora sem justificativa)

6. **[Sugestão de visual — capa]** `[CAPA] - Como capa podemos construir uma imagem de paper cut que mostre uma cena de transformação do agro para o banco`
   Resolução: este pipeline não produz mais capa/ilustração (nota de 2026-09-09 do sistema). Registrado como **tensão** na etapa 1, para o gate humano (etapa 10) decidir o que fazer com a ideia.

7. **[Sugestão de visual]** `[diag-01: fluxo produtor/cooperativa ← banco ← investidor, com as duas relações de crédito rotuladas separadamente]`
   Resolução: (etapa 8 — `prompts-visuais`)

8. **[Nota de conteúdo]** `[VERIFICAR: direcionamento de 60% (Res. CMN 5.216/2025) e se a Res. CMN 5.315/2026 alterou percentuais; reconferir no MCR na semana da publicação]`
   Resolução: (etapa 7 — `verificador-tecnico`)

9. **[Instrução de escrita]** Nota do autor: decidir se entra o parágrafo sobre 2026 (estoque de LCA caindo 4% no 1º semestre enquanto LCI sobe 20%; taxa média da LCA de 12 meses de 90,05% para 88,24% do CDI; fontes B3 e Quantum Finance).
   Resolução: (etapa 4 — decisão de inclusão na integração do rascunho)

10. **[Sugestão de visual]** `[graf-01: prazo no eixo X; duas linhas — regra linear (em degraus) e break-even real (descendente); a área entre elas é o erro]`
    Resolução: (etapa 8 — `prompts-visuais`)

11. **[Nota de conteúdo]** `[VERIFICAR: CDI vigente na data de publicação; recalcular a tabela. 6 meses = 126 d.u. ≈ 182 dias corridos, faixa de 20%]`
    Resolução: (etapa 7 — `verificador-tecnico`)

12. **[Instrução de escrita]** Nota do autor: LCA de cooperativa de crédito é coberta pelo FGCoop, não pelo FGC — decidir se entra em uma frase.
    Resolução: (etapa 4)

13. **[Nota de conteúdo]** `[VERIFICAR: teto global do FGCoop, se a menção entrar]` — condicional ao item 12.
    Resolução: (etapa 7, só se o item 12 entrar)

14. **[Nota de conteúdo]** `[VERIFICAR: número da resolução do CMN de maio/2025]` (prazo mínimo de 6 meses)
    Resolução: (etapa 7)

15. **[Instrução de escrita]** Nota do autor: MP 1.303/2025 (5% de IR sobre novas emissões, caducou 8/10/2025) e IRPFM (Lei 15.270/2025, preserva LCA fora da base, relevante só para alta renda — "talvez fique para o texto 'Por quanto tempo vale ser isento?'"). Decidir se entra.
    Resolução: (etapa 4 — inclui componente de verificação factual dos números de lei/MP, a levar à etapa 7 se entrar)

16. **[Instrução de escrita]** `[VERIFICAR: numa LCA em % do CDI os componentes não se somam literalmente; deixar explícito que a decomposição é conceitual ou reescrever em termos de spread sobre a curva]`
    Resolução: (etapa 4 — ajuste de prosa, não só verificação factual)

17. **[Instrução de escrita]** `[VERIFICAR: com 60% de direcionamento, nem todo real captado vai para a carteira a CDI + 4%; ajustar o exemplo ou explicitar a premissa]`
    Resolução: (etapa 4)

18. **[Instrução de escrita]** Nota do autor: manter ou não a pergunta ao leitor sobre qual curva pesa mais (LF do mesmo banco, CDBs comparáveis ou LCAs de bancos pares) — "funciona como exercício e abre comentário".
    Resolução: (etapa 4)

19. **[Nota de conteúdo]** `[VERIFICAR: edição do Fabozzi e referência completa do artigo do Caffagni]`
    Resolução: (etapa 7)
