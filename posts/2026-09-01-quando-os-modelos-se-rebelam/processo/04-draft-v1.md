# Draft v1 — Quando os Modelos se Rebelam (rodada 4)

Nota de processo: a Camada 1 (material do próprio autor) é praticamente o texto inteiro — o
rascunho já chegou como prosa quase final, com duas instruções de escrita pontuais. Este draft
preserva o texto do autor verbatim onde não há instrução, e resolve as duas instruções abaixo,
marcadas inline com `<!-- NOVO (etapa 4): ... -->` para facilitar a leitura desta etapa —
os comentários somem na consolidação (etapa 9).

Placeholders de visual (`ilu-01`, `graf-01`, `diag-01`) inseridos nos pontos decididos em
...
[VERIFICAR: a atribuição desta fala a Peter Kempthorne especificamente não se sustentou em
três rodadas de pesquisa/verificação anteriores — a página oficial do MIT OCW atribui a aula
mais provável (Lecture 3, Probability Theory) a Choongbum Lee, não a Kempthorne, e a
transcrição da aula não contém a frase; o mesmo vale para as quatro aulas mais prováveis de
Kempthorne por tema. A etapa 7 desta rodada refaz a checagem — se não confirmar, a alternativa
já validada é "um dos professores diz" sem nome próprio, mantendo a citação entre aspas
diretas.]
...
Em finanças, o que existe são modelos. Não teorias — pelo menos não no sentido em que Derman usa a palavra. CAPM, Black–Scholes, Hipótese dos Mercados Eficientes, Vasicek, Gordon: todos imitam o _estilo_ da física e usam a _sintaxe_ da matemática, mas não têm a mesma ancoragem ontológica.
...
existe, no sentido forte, é uma teoria _empírica_ sobre o próprio mercado. O teorema é
...
- Joesley Day: InfoMoney e Seu Dinheiro, pregão de 18/05/2017; Banco Central (SGS série 1, PTAX)
- Aracruz e Sadia: Bloomberg, "Aracruz Fails to Settle $2.13 Billion Derivative Loss" (12/12/2008); Revista RACEF, _A crise financeira internacional (2008) e o efeito dos derivativos cambiais_; BIS Quarterly Review (jun/2009) e BIS Papers No 54 (Mesquita & Torós, dez/2010)
